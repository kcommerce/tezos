...
module Stubs = struct
  ...
  external get : fr -> fr_array -> int -> unit = "caml_get_stubs"
  ...
  external rescale : fr_array -> fr_array -> int -> int -> unit
    = "caml_evaluations_rescale_stubs"
    [@@noalloc]
  ...
  external mul_arrays :
    ...
    int ->
    int ->
    unit = "caml_evaluations_mul_arrays_stubs"
    [@@noalloc]
  ...
  external linear_arrays :
    ...
    = "caml_evaluations_linear_arrays_stubs"
    [@@noalloc]
  ...
  let mul_c ~evaluations
    ...
    let (deg_result, list_array, is_zero, (name_min, min_length_eval)) =
      ...
      let rec fold_left3 f acc l1 l2 l3 =
        ...
        | ([], [], []) -> acc
        | (a1 :: l1, a2 :: l2, a3 :: l3) ->
            fold_left3 f (f acc a1 a2 a3) l1 l2 l3
        | _ ->
            raise
